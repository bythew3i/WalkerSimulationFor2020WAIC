...
void Node::Init () {
  ...
  if (publish_pointcloud_param_) {
    ...
    map_points_current_publisher_ = node_handle_.advertise<sensor_msgs::PointCloud2> (name_of_node_+"/map_points_current", 1);
  }
...
void Node::Update () {
  ...
  if (publish_pointcloud_param_) {
    ...
    PublishMapPoints_CurrentFrame (orb_slam_ ->GetTrackedMapPoints());
  }
}
...
void Node::PublishMapPoints_CurrentFrame (std::vector<ORB_SLAM2::MapPoint*> map_points) {
  sensor_msgs::PointCloud2 cloud = CurrentMapPointsToPointCloud (map_points);
  map_points_current_publisher_.publish (cloud);
}
...
sensor_msgs::PointCloud2 Node::CurrentMapPointsToPointCloud (std::vector<ORB_SLAM2::MapPoint*> map_points) {
  if (map_points.size() == 0) {
    std::cout << "Map point vector is empty!" << std::endl;
  }
  ...
  cv::Mat Tcw = orb_slam_ ->GetCurrentPosition(); // Converthttps://github.com/raulmur/ORB_SLAM2/issues/226
  if (Tcw.empty())
  {
  ...
  int point_cnt = 0;
  for (unsigned int i=0; i<map_points.size(); i++) {
    if (map_points.at(i) && map_points.at(i)->nObs >= min_observations_per_point_) {
      cv::Mat p = map_points.at(i)->GetWorldPos();
      cv::Mat tp = Rwc * p + twc;
      data_array[0] = tp.at<float> (2); //x. Do the transformation by just reading at the position of z instead of x
      data_array[1] = -1.0* tp.at<float> (0); //y. Do the transformation by just reading at the position of x instead of y
      data_array[2] = -1.0* tp.at<float> (1); //z. Do the transformation by just reading at the position of y instead of z
      //TODO dont hack the transformation but have a central conversion function for MapPointsToPointCloud and TransformFromMat

      memcpy(cloud_data_ptr+(point_cnt*cloud.point_step), data_array, num_channels*sizeof(float));
      point_cnt++;
    }
  }
